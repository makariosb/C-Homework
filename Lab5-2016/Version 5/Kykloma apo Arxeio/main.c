...
/*Το προγραμμα δουλευει για κυκλωματα με 1 εξοδο.
	...
	Σε δεύτερο αρχείο εισάγονται οι τιμές των εισόδων/εξόδων, με την ίδια μορφή όπως στο ερώτημα 3 (εκτός των 2 πρωτων σειρών προφανως)
	
	{μπορείτε να βρείτε το πρότυπο του κυκλωματος του σχήματος 1 της εκφώνησης στο αρχείο "kykloma1.txt" και το αρχείο 
	εισόδων/εξόδων του ειναι το "testinputs1.txt"}
*/
...
int main() {
	...
	while ((fgets(buffer,sizeof(buffer),sourcefile))!=NULL){
		...
		if (nextgate(*gatelist[gatenum-1],buffer,&pos)!=(buffer[2*(inputs+outputs-1)]-'0')){
			printf("Did not calculate the correct output, refference string: %s \n",buffer);
		}
	}
	puts("Done!");
	return 0;
